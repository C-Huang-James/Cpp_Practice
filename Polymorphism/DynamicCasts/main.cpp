...
int main(){
    ...
    // dog_ptr->do_some_dog_thingy(); // This will cause undefined behavior and crash
    if(dog_ptr){
	...
	//dog_ref.do_some_dog_thingy();

    std::cout << "---------------" << std::endl;

    //Doing proper checks with references
	Dog* dog_ref_1 {dynamic_cast<Dog*>(&animal_ref)};
	if(dog_ref_1){
		dog_ref_1->do_some_dog_thingy();
	}else{
